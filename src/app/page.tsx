  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative min-h-[90vh] flex flex-col items-center justify-center text-center px-6 pt-32 pb-20 overflow-hidden">
        ...
        />
        <span className="text-[.72rem] font-semibold tracking-[.32em] uppercase text-accent mb-5 flex items-center gap-3">
          <span className="w-8 h-px bg-accent" /> Shakargarh&apos;s First Luxury Family Spot{" "}
          <span className="w-8 h-px bg-accent" />
        </span>
        <h1 className="font-display font-semibold text-[clamp(2.6rem,7vw,5.5rem)] leading-[1.05] mb-6">
          Crafted To Satisfy
          <br />
          <em className="text-gradient not-italic italic">Every Craving</em>
        </h1>
        <p className="text-muted max-w-xl text-lg font-light mb-10">
          Handcrafted gourmet burgers and wood-fired pizzas made with premium ingredients —
          order straight from the menu below.
        </p>
        <a
          href="#menu"
          className="px-9 py-4 rounded-full bg-accent text-black text-sm font-semibold tracking-widest uppercase hover:scale-105 transition-transform"
        >
          Order Now
        </a>
      </section>

      <div id="menu" className="max-w-6xl mx-auto px-6 pb-40">
        {cats.map((cat) => (
          <MenuSection
            key={cat.id}
            category={cat}
